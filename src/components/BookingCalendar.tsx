...
import { Calendar, Clock, Loader2, User, Mail, CheckCircle } from 'lucide-react';
...
export default function BookingCalendar() {
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show success immediately after webhook response
      setSuccess(true);
      setSelectedCabin('');
      setSelectedTime('');
      setTimeSlots([]);
      ...
      createBooking({
        ...
        userEmail: email,
        status: 'confirmed',
      }).catch(console.error); // Handle silently in background
    } catch (err) {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-2xl mx-auto text-center">
        ...
        <button
          ...
          className="px-6 py-3 bg-elida-gold text-white rounded-xl font-medium hover:bg-elida-accent transition-colors duration-300"
        >
